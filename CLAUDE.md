...
| `src/memory.ts` | Pure memory budgeting (#271): `parseMemoryMb`, `containerMemoryMb`, `jvmOverheadMb`, `heapBudgetProblem`. The container cap and the JVM heap are two settings for the same RAM — the kernel enforces the cap and the JVM *commits* the heap, so a heap that does not fit is a container killed mid-save. Mirrored (loosely) by `dashboard/src/memory.ts` for the live warning |
